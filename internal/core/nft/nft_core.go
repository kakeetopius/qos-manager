...
func (c *NFT) initChains() error {
	...
	forwardOutChain, err := lookupQosChain(c.conn, chainParams{
		table:       c.QosTable.Table,
		chainName:   FORWARDOUTCHAINNAME,
	...
	c.initRules(&forwardOutChain)
	c.QosTable.ForwardOutChain = forwardOutChain

	forwardInChain, err := lookupQosChain(c.conn, chainParams{
		table:       c.QosTable.Table,
		chainName:   FORWARDINCHAINNAME,
		hook:        nftables.ChainHookForward,
		chainPolicy: nftables.ChainPolicyAccept,
	}, &c.opts)
	if err != nil {
...
func (c *NFT) initRules(chain *QosChain) error {
	...
	if chain.Name == FORWARDINCHAINNAME {
		// if are working with the forward chain the matches based on source paramaters the keys are reversed to match source ip / port
